...
| `/seo everything [path]` | Autonomous full-project SEO pass over a codebase: perceive, fan out specialists, apply every relevant fix on a branch, show one diff |
...
![Claude SEO audit signal flow: /seo audit enters the orchestrator, fans out to 26 sub-skills and up to 15 parallel audit agents, and converges through the scoring engine into a prioritized report](assets/signal-flow.svg)
...
**Heavy client-side hydration timing.** The headless renderer handles most SPAs out of the box (`--render auto` detects empty `<div id="root">` shells and switches to Playwright). It also **scrolls the page by default** (`--settle scroll`) so content tied to scroll position or IntersectionObserver hydrates before capture, and `--settle network` adds a bounded network-idle wait for race-prone third-party widget mounts. Content hidden behind a click — modal opens, tab panels, accordions — is captured with the opt-in `--reveal-hidden` flag, which expands `<details>` and `aria-expanded` controls without following links or submitting forms. The residual edge case is content gated behind interactions `--reveal-hidden` won't perform (a form submission, a multi-step flow, a hover-only menu); for those, manually triggering the `seo-visual` subagent and comparing its Playwright snapshot to the raw-HTML subagents' findings is the recommended workflow.
